...
from customer.utils import send_otp_via_email, send_otp_via_phone
...
from app1.models import OTP, Complaint, Country_Codes, CustomerReview, Invoice, Payment, ServiceProvider, ServiceRegister, ServiceRequest, User
from service_provider.permissions import IsOwnerOrAdmin
from .serializers import ComplaintSerializer, CustomerReviewSerializer, CustomerServiceRequestSerializer, DeclineServiceRequestSerializer, InvoiceSerializer, PaymentListSerializer, ResendOTPSerializer, ServiceDetailsSerializer, ServiceProviderPasswordForgotSerializer, ServiceRegisterSerializer, ServiceRegisterUpdateSerializer, ServiceRequestCustomSerializer, ServiceRequestSerializer, SetNewPasswordSerializer, ServiceProviderLoginSerializer,ServiceProviderSerializer
        ...
        try:
            ...
            if '@' in email_or_phone:
                ...
                try:
                    ...
                    number = str(fullnumber.national_number)
                    user = User.objects.get(phone_number=number, country_code=country_code, is_service_provider=True)
...
class CustomerServiceRequestView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            service_request = ServiceRequest.objects.get(booking_id=booking_id, service_provider=service_provider.user)
...
class ServiceRequestInvoiceView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        try:
            ...
            print(service_request.reschedule_status)
...
class ServiceDetailsView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        return ServiceRequest.objects.select_related('service_provider', 'service', 'customer').prefetch_related('servicerequests_invoices')

    def post(self, request, *args, **kwargs):
        booking_id = request.data.get('booking_id')  # Get the pk from the request body
        print(booking_id)
        if not booking_id:
            return Response({"error": "No booking_id provided."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        service_request_data = self.get_serializer(service_request).data

        # Fetch and serialize the related invoices, if any
        invoices = Invoice.objects.filter(service_request=service_request)
        invoice_serializer = InvoiceSerializer(invoices, many=True)
        
        # Add the invoices to the response data
        service_request_data['invoices'] = invoice_serializer.data

        return Response(service_request_data, status=status.HTTP_200_OK)
...
class DeclineServiceView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            return Response({"detail": "No ServiceRequest matches the given booking ID."}, status=status.HTTP_404_NOT_FOUND)
        ...
        #Check if a payment has been made for this service request
        try:
            invoice = Invoice.objects.get(service_request=service_request)
            if invoice.payment_status in ['paid', 'partially_paid']:
                return Response({
                    "error": "Cannot decline the service request as the payment status is either 'paid' or 'partially paid'."
                }, status=status.HTTP_400_BAD_REQUEST)
        except Invoice.DoesNotExist:
            pass  # No invoice found, continue with decline logic
...
class DeductLeadBalanceView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        try:
            booking_id = request.data.get('booking_id')  # Retrieve booking_id from the request data
...
class ComplaintViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            user = request.user
            booking_id = request.data.get('service_request_booking_id')
            service_request = ServiceRequest.objects.get(booking_id=booking_id)
...
class ServiceProviderReviews(APIView):
    ...
    def get(self, request):
        ...
        if total_reviews == 0:
            return Response({
                'message': "No reviews available for this service provider.",
                'average_rating': 0,
                'total_reviews': 0,
                'rating_scale': "No Rating" 
            })

        # Calculate the average rating if reviews present
