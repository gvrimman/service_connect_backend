...
from app1.models import Complaint, Country_Codes, CustomerReview, DeclineServiceModel, Invoice, Payment, ServiceProvider, ServiceRegister, ServiceRequest, Subcategory, User  
    ...
    def validate_email_or_phone(self, value):
        ...
        if '@' in value:
            ...
            try:
                ...
                if not User.objects.filter(phone_number=number, country_code=country_code, is_service_provider=True).exists():
...
class CustomerServiceRequestSerializer(serializers.ModelSerializer):
    serviceprovider_name = serializers.CharField(source='service_provider.full_name', read_only = True)
    provider_place  = serializers.CharField(source='service_provider.place', read_only = True)
    provider_district  = serializers.CharField(source='service_provider.district', read_only = True)
    subcategory = serializers.CharField(source='service.subcategory', read_only=True)
    ...
    customer_place = serializers.CharField(source='customer.place', read_only=True)
    ...
    class Meta:
        model = ServiceRequest
        fields = [
            'booking_id','serviceprovider_name','provider_district','provider_place','title', 'subcategory', 'description', 
            'acceptance_status', 'availability_from', 'availability_to', 'image', 
            'profile_image','customer_name', 'customer_address','customer_landmark',
            'customer_pincode','customer_phone','customer_email','customer_place'
        ]
